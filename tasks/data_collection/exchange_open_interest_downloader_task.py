...
class ExchangeOpenInterestDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Exchange Open Interest downloader")
        ...
        try:
            ...
            for trading_pair in trading_pairs:
                ...
                table_name=f"coinglass_{trading_pair.replace('-','')}_option_exchange_oi_history_1h"
                symbol = trading_pair.split("-")[0]  # Only need base currency for this endpoint
                url = "https://open-api-v3.coinglass.com/api/option/v2/exchange-oi-history"
                headers = {
                ...
                if response.status_code != 200:
                    logging.error(f"Error fetching Exchange Open Interest data for {symbol}: {response.text}")
                    continue

                data = response.json()
                if not data.get("data"):
                    logging.info(f"No Exchange Open Interest data available for {symbol}")
                    continue
                try:
                ...
                for index in range(len(data['data']['dateList'])):                    
                    if(data["data"]["dateList"][index]>last_timestamp):                                            
                        await timescale_client.insert_exchange_open_interest(
                            table_name=table_name,
                            symbol=trading_pair,
                            provider="coinglass",
                ...
                logging.info(f"{now} - Inserted Exchange Open Interest data for {symbol}")
                await asyncio.sleep(1)

        except Exception as e:
            logging.exception(f"{now} - Error processing Exchange Open Interest data: {e}")
...
if __name__ == "__main__":
    ...
    task = ExchangeOpenInterestDownloaderTask(
        "Exchange Open Interest Downloader",
        timedelta(hours=1),
        config
    )
