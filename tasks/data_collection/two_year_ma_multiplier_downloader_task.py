...
class TwoYearMAMultiplierDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Two Year MA Multiplier downloader")
        ...
        try:
            table_name="coinglass_tow_year_ma_multiplier_1h"

            url = "https://open-api-v3.coinglass.com/api/index/tow-year-ma-multiplier"
            headers = {
            ...
            if response.status_code != 200:
                logging.error(f"Error fetching Two Year MA Multiplier data: {response.text}")
                return

            data = response.json()
            if not data.get("data"):
                logging.info("No Two Year MA Multiplier data available")
                return

            for element in data["data"]:
                await timescale_client.insert_two_year_ma_multiplier(
                    table_name=table_name,
                    provider="coinglass",
                    buy_qty=element["buyQty"],
                    price=element["price"],
                    ma730_mu5=element["mA730Mu5"],
                    ma730=element["mA730"],
            ...
            logging.info(f"{now} - Inserted Two Year MA Multiplier data")

        except Exception as e:
            logging.exception(f"{now} - Error processing Two Year MA Multiplier data: {e}")
...
if __name__ == "__main__":
    ...
    task = TwoYearMAMultiplierDownloaderTask(
        "Two Year MA Multiplier Downloader",
        timedelta(hours=1),
        config
    )
