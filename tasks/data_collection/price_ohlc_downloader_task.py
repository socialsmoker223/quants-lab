...
class PriceOHLCDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Price OHLC downloader")
        ...
        try:
            table_name="coinglass_spot_ohlc_1h_high_freq_1h"
            ...
            for trading_pair in trading_pairs:
                ...
                if response.status_code != 200:
                    logging.error(f"Error fetching Price OHLC data for {symbol}: {response.text}")
                    continue

                data = response.json()
                if not data.get("data"):
                    logging.info(f"No Price OHLC data available for {symbol}")
                    continue

                for element in data["data"]:
                    await timescale_client.insert_price_ohlc(
                        table_name=table_name,
                        symbol=trading_pair,
                        exchange=self.connector_name.replace("_perpetual", ""),
                        provider="coinglass",
                ...
                logging.info(f"{now} - Inserted Price OHLC data for {symbol}")
                await asyncio.sleep(1)

        except Exception as e:
            logging.exception(f"{now} - Error processing Price OHLC data: {e}")
...
if __name__ == "__main__":
    ...
    task = PriceOHLCDownloaderTask(
        "Price OHLC Downloader",
        timedelta(hours=1),
        config
    )
