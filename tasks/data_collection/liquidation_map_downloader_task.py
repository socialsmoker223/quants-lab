...
class LiquidationMapDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Liquidation Map downloader")
        ...
        try:
            table_name="coinglass_futures_liquidation_map_1h"
            ...
            for trading_pair in trading_pairs:
                ...
                params = {
                    "symbol": symbol,
                    "range": "1d",  # Default to 1d interval
                    "exchange": self.connector_name.replace("_perpetual", "")
                }

                response = requests.get(url, headers=headers, params=params)
                if response.status_code != 200:
                    logging.error(f"Error fetching Liquidation Map data for {symbol}: {response.text}")
                    continue

                data = response.json()
                if not data.get("data"):
                    logging.info(f"No Liquidation Map data available for {symbol}")
                    continue

                for element in data["data"]:
                    await timescale_client.insert_liquidation_map(
                ...
                logging.info(f"{now} - Inserted Liquidation Map data for {symbol}")
                await asyncio.sleep(1)

        except Exception as e:
            logging.exception(f"{now} - Error processing Liquidation Map data: {e}")
...
if __name__ == "__main__":
    ...
    task = LiquidationMapDownloaderTask(
        "Liquidation Map Downloader",
        timedelta(hours=1),
        config
    )
