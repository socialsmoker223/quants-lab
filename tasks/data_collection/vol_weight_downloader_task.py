...
class VolWeightDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Volume Weight downloader")
        ...
        try:
            table_name="coinglass_futures_vol_weight_ohlc_1h_high_freq_1h"
            ...
            for trading_pair in trading_pairs:
                ...
                url = "https://open-api-v3.coinglass.com/api/futures/fundingRate/vol-weight-ohlc-history"
                headers = {
                ...
                if response.status_code != 200:
                    logging.error(f"Error fetching Volume Weight data for {symbol}: {response.text}")
                    continue

                data = response.json()
                if not data.get("data"):
                    logging.info(f"No Volume Weight data available for {symbol}")
                    continue

                for element in data["data"]:
                    await timescale_client.insert_vol_weight(
                        table_name=table_name,
                        symbol=trading_pair,
                ...
                logging.info(f"{now} - Inserted Volume Weight data for {symbol}")
                await asyncio.sleep(1)

        except Exception as e:
            logging.exception(f"{now} - Error processing Volume Weight data: {e}")
...
if __name__ == "__main__":
    ...
    task = VolWeightDownloaderTask(
        "Volume Weight Downloader",
        timedelta(hours=1),
        config
    )
