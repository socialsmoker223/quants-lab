...
class LiquidationDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting Liquidation downloader")
        ...
        try:
            table_name="coinglass_futures_liquidation_1h_1h"

            trading_pairs = self.config.get("trading_pairs", ["BTC-USDT"])

            for trading_pair in trading_pairs:
                symbol = trading_pair.replace("-", "")  # Convert BTC-USDT to BTCUSDT
                url = "https://open-api-v3.coinglass.com/api/futures/liquidation/v2/history"
                headers = {
                ...
                if response.status_code != 200:
                    logging.error(f"Error fetching Liquidation data for {symbol}: {response.text}")
                    continue

                data = response.json()
                if not data.get("data"):
                    logging.info(f"No Liquidation data available for {symbol}")
                    continue

                for element in data["data"]:
                    await timescale_client.insert_liquidation(
                        table_name=table_name,
                        symbol=trading_pair,
                        exchange=self.connector_name.replace("_perpetual", ""),
                        provider="coinglass",
                ...
                logging.info(f"{now} - Inserted Liquidation data for {symbol}")
                await asyncio.sleep(1)

        except Exception as e:
            logging.exception(f"{now} - Error processing Liquidation data: {e}")
...
if __name__ == "__main__":
    ...
    task = LiquidationDownloaderTask(
        "Liquidation Downloader",
        timedelta(hours=1),
        config
    )
